...
Консольде `Using an unspecified index` ескертуін жою және admin/notifications қауіпсіздігін
қамтамасыз ету үшін, Firebase Console → Realtime Database → Rules бөліміне жобамен бірге
келген `firebase-database-rules.json` файлының мазмұнын қойыңыз. Бұл:

- `orders/userId` мен `orders/status` бойынша индекс қосады (консоль ескертуін жояды,
  сұраулар жылдамдайды)
- `admins` түйінін клиент жағынан жазуға тыйым салады (тек Firebase Console арқылы қолмен
  өзгертіледі — осылай ешкім өзін-өзі admin ете алмайды)
- `notifications/{uid}` түйінін тек сол пайдаланушының өзі оқи алатындай шектейді
